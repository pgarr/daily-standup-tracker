...
**S-05: Team Lead views team feed with confirmed blocker alerts** — the north star (the smallest end-to-end slice whose successful delivery proves the core product hypothesis) is that the system correctly detects recurring blockers, surfaces them through member confirmation, and makes them visible in a shared team feed. Every other slice exists to enable this one.
...
| C      | Entry mutations      | `S-06`                                        | Branches from S-03, parallel with S-04; not on the critical path to the north star.  |
...
- **Blockers:** —
- **Unknowns:** —
- **Risk:** workspace creation must wire correctly with the F-01 schema; this is the root of the entire auth + access control chain — a schema mismatch here propagates through every downstream slice
- **Status:** done
...
- **Outcome:** member can edit or delete their own previously submitted standup entry.
- **Change ID:** standup-entry-edit-delete
...
|------------|--------------------------------|---------------------------------------------------------------------|-----------------------|-----------------------------------------------------|
| F-01       | workspace-member-schema        | Add workspace + workspace_member schema with RLS                    | no                    | Done — archived                                     |
| S-01       | auth-and-workspace             | Auth flow + workspace creation + Team Lead role                     | no                    | Done — archived                                     |
| S-02       | member-invite-and-join         | Email invite + member registration + workspace join                 | yes                   | Run `/10x-plan member-invite-and-join`; parallel with S-03 |
| S-03       | standup-submission-and-history | Standup form + history list + business-day streak                   | yes                   | Run `/10x-plan standup-submission-and-history`; parallel with S-02 |
| S-04       | blocker-detection-flow         | Blocker similarity detection + confirmation UI + alert recording    | no                    | Requires S-03; parallel with S-06                   |
| S-05       | team-feed-and-alerts           | Team feed + confirmed alerts + threshold config ⭐ north star       | no                    | Requires S-01, S-02, S-03, S-04                     |
| S-06       | standup-entry-edit-delete      | Edit and delete own standup entry                                   | no                    | Requires S-03; parallel with S-04                   |

## Open Roadmap Questions

No blocking questions remain — all PRD v3 Open Questions were resolved in prd-v2.md (blocker matching: similarity + member confirmation; streak: business days only, Mon–Fri).
...
4. **Does deleting a standup entry retroactively adjust the streak counter?** — Owner: user. Block: S-06 implementation detail only.
...
- **FR-010: Full-text search of history** — Why parked: nice-to-have per PRD v3; useful only at 30+ entries; date filter sufficient for v1.
