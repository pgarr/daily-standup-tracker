...
- [x] 2.1 `npx playwright test` exits 0 (all protected routes 302, all public routes non-302) — 18c9944
- [x] 2.2 `npm run lint` passes with new config + spec in place — 18c9944

#### Manual

- [x] 2.3 `playwright test --reporter=list` output is legible (test names + routes visible) — 18c9944
- [x] 2.4 Remove `/dashboard` from `AUTH_REQUIRED_ROUTES`, run `playwright test`, confirm failure; revert — 18c9944
...
- [x] 3.1 CI job passes on a new PR: lint → npm test → playwright install → playwright test → build all green — 2ea3493

#### Manual

- [x] 3.2 Comment out one Playwright assertion, push to branch, confirm CI fails on playwright step (not build step); revert — 2ea3493
